...
// Prepare the data for the first chart
// Prepare the data for the first chart
const chartData = response.data.map((employee6) => ({
  name: formatDate(employee6.datedesortie6), // Use datedesortie for x-axis
  y: parseFloat(employee6.consmyne6), // Convert to a number // Use datedesortie for y-axis
  consmyne: ((employee6.quantitetotal6 - employee6.stabilite6) / employee6.jourautono6 / 1000).toFixed(2),
}));
setChartData(chartData);
        ...
        const chartData2 = response.data.map((employee6) => ({
          name: employee6.datedesortie6,  // Change here
          y: parseFloat(employee6.consmyne6), // Convert to a number
          consmyne6: (employee6.consmyne6),
        }));
  ...
  const handleExportChart = () => {
    const chartOptions = {
      ...
      series: [
        ...
        {
          name: 'consmyne',
          data: chartData.map((dataPoint) => dataPoint.consmyne6),
        },
      ],
    };
    ...
    const chartOptions2 = {
      ...
      series: [
        ...
        },
        {
          ...
          dataLabels: {
            enabled: true,
            format: '{point.consmyne6}',
          },
        },
      ],
      tooltip: {
        formatter: function () {
          return `<span style="color: ${this.color};">&#9679;</span> consmyne6: ${this.point.consmyne6}`;
        },
        style: {
          color: 'black',
          fontSize: '17px',
      ...
      },
      {
        ...
        data: chartData2.map((dataPoint, index) => {
          const formattedDataPoint = {
            x: index,
            y: dataPoint.y,
            consmyne6: dataPoint.consmyne6, // Use consmyne instead of name
          };
          // Set tooltip content when the point is clicked
          formattedDataPoint.events = {
            click: function () {
              setTooltipContent(`consmyne6: ${dataPoint.consmyne6}`);
            },
          };
        ...
        dataLabels: {
          enabled: true,
          format: '{point.consmyne6}', // Display consmyne in dataLabels
        },
      },
    ],
    tooltip: {
      ...
      formatter: function () {
        ...
        return  `<span style="color: ${this.color};">&#9679;</span> cons.Myne: ${this.point.consmyne6}`|| tooltipContent || `consmyne: ${this.point.consmyne6}`;
      },
      style: {
...
export default DataGraphMaranda3;
