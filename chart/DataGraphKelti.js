...
// Prepare the data for the first chart
// Prepare the data for the first chart
const chartData = response.data.map((employee1) => ({
  name: formatDate(employee1.datedesortie1), // Use datedesortie for x-axis
  y: parseFloat(employee1.prixdegazoil1), // Convert to a number // Use datedesortie for y-axis
  consmyne1: employee1.consmyne1,
}));
setChartData(chartData);
        ...
        const chartData2 = response.data.map((employee1) => ({
            name: employee1.datedesortie1,  // Change here
            y: parseFloat(employee1.prixdegazoil1), // Convert to a number
            consmyne1: ((employee1.quantitetotal1 - employee1.stabilite1) / employee1.jourautono1 / 1000).toFixed(2),
          }));
  ...
  const handleExportChart = () => {
    const chartOptions = {
      ...
      series: [
        ...
        {
          name: 'consmyne',
          data: chartData.map((dataPoint) => dataPoint.consmyne1),
        },
      ],
    };
    ...
    const chartOptions2 = {
      ...
      series: [
        ...
        },
        {
          ...
          dataLabels: {
            enabled: true,
            format: '{point.consmyne1}',
          },
        },
      ],
      tooltip: {
        formatter: function () {
          return `<span style="color: ${this.color};">&#9679;</span> consmyne1: ${this.point.consmyne1}`;
        },
        style: {
          color: 'black',
          fontSize: '17px',
      ...
      },
      {
        ...
        data: chartData2.map((dataPoint, index) => {
          const formattedDataPoint = {
            x: index,
            y: dataPoint.y,
            consmyne1: dataPoint.consmyne1, // Use consmyne instead of name
          };
          // Set tooltip content when the point is clicked
          formattedDataPoint.events = {
            click: function () {
              setTooltipContent(`consmyne1: ${dataPoint.consmyne1}`);
            },
          };
        ...
        dataLabels: {
          enabled: true,
          format: '{point.consmyne1}', // Display consmyne in dataLabels
        },
      },
    ],
    tooltip: {
      ...
      formatter: function () {
        ...
        return  `<span style="color: ${this.color};">&#9679;</span> cons.Myne: ${this.point.consmyne1}`|| tooltipContent || `consmyne1: ${this.point.consmyne1}`;
      },
      style: {
