...
// Prepare the data for the first chart
// Prepare the data for the first chart
const chartData = response.data.map((employee11) => ({
  name: formatDate(employee11.datedesortie11), // Use datedesortie for x-axis
  y: parseFloat(employee11.consmyne11), // Convert to a number // Use datedesortie for y-axis
  consmyne:employee11.consmyne11,
}));
setChartData(chartData);
        ...
        const chartData2 = response.data.map((employee11) => ({
          name: employee11.datedesortie11,  // Change here
          y: parseFloat(employee11.consmyne11), // Convert to a number
          consmyne11: employee11.consmyne11,
        }));
  ...
  const handleExportChart = () => {
    const chartOptions = {
      ...
      series: [
        ...
        {
          name: 'consmyne',
          data: chartData.map((dataPoint) => dataPoint.consmyne11),
        },
      ],
    };
      ...
      series: [
        ...
        },
        {
          ...
          data: chartData2.map((dataPoint, index) => ({
            ...
            y: dataPoint.y,
            consmyne11: dataPoint.consmyne11,
          })),
          type: 'line',
          ...
          dataLabels: {
            enabled: true,
            format: '{point.consmyne11}',
          },
        },
      ],
      tooltip: {
        formatter: function () {
          return `<span style="color: ${this.color};">&#9679;</span> consmyne11: ${this.point.consmyne11}`;
        },
        style: {
          color: 'black',
          fontSize: '17px',
      ...
      },
      {
        ...
        data: chartData2.map((dataPoint, index) => {
          const formattedDataPoint = {
            x: index,
            y: dataPoint.y,
            consmyne11: dataPoint.consmyne11, // Use consmyne instead of name
          };
          // Set tooltip content when the point is clicked
          formattedDataPoint.events = {
            click: function () {
              setTooltipContent(`consmyne11: ${dataPoint.consmyne11}`);
            },
          };
        ...
        dataLabels: {
          enabled: true,
          format: '{point.consmyne11}', // Display consmyne in dataLabels
        },
      },
    ],
    tooltip: {
      ...
      formatter: function () {
        ...
        return  `<span style="color: ${this.color};">&#9679;</span> cons.Myne: ${this.point.consmyne11}`|| tooltipContent || `consmyne: ${this.point.consmyne11}`;
      },
      style: {
...
export default DataGraphGorgues;
