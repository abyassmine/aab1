...
// Prepare the data for the first chart
// Prepare the data for the first chart
const chartData = response.data.map((employee15) => ({
  name: formatDate(employee15.datedesortie15), // Use datedesortie for x-axis
  y: parseFloat(employee15.consmyne15), // Convert to a number // Use datedesortie for y-axis
  consmyne: ((employee15.quantitetotal15 - employee15.stabilite15) / employee15.jourautono15 / 1000).toFixed(2),
}));
setChartData(chartData);
        ...
        const chartData2 = response.data.map((employee15) => ({
          name: employee15.datedesortie15,  // Change here
          y: parseFloat(employee15.consmyne15), // Convert to a number
          consmyne15: (employee15.consmyne15),
        }));
  ...
  const handleExportChart = () => {
    const chartOptions = {
      ...
      series: [
        ...
        {
          name: 'consmyne',
          data: chartData.map((dataPoint) => dataPoint.consmyne15),
        },
      ],
    };
      ...
      series: [
        ...
        },
        {
          ...
          data: chartData2.map((dataPoint, index) => ({
            ...
            y: dataPoint.y,
            consmyne15: dataPoint.consmyne15,
          })),
          type: 'line',
          ...
          dataLabels: {
            enabled: true,
            format: '{point.consmyne15}',
          },
        },
      ],
      tooltip: {
        formatter: function () {
          return `<span style="color: ${this.color};">&#9679;</span> consmyne15: ${this.point.consmyne15}`;
        },
        style: {
          color: 'black',
          fontSize: '17px',
      ...
      },
      {
        ...
        data: chartData2.map((dataPoint, index) => {
          const formattedDataPoint = {
            x: index,
            y: dataPoint.y,
            consmyne15: dataPoint.consmyne15, // Use consmyne instead of name
          };
          // Set tooltip content when the point is clicked
          formattedDataPoint.events = {
            click: function () {
              setTooltipContent(`consmyne15: ${dataPoint.consmyne15}`);
            },
          };
        ...
        dataLabels: {
          enabled: true,
          format: '{point.consmyne15}', // Display consmyne in dataLabels
        },
      },
    ],
    tooltip: {
      ...
      formatter: function () {
        ...
        return  `<span style="color: ${this.color};">&#9679;</span> cons.Myne: ${this.point.consmyne15}`|| tooltipContent || `consmyne: ${this.point.consmyne15}`;
      },
      style: {
...
export default DataGraphMaranda1;
